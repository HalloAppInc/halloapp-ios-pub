...
struct SliderMedia {
    ...
    init(image: UIImage?, type: ChatMessageMediaType) {
        self.image = image
        self.type = type
    }
}
...
class ChatMediaSlider: UIView, UIScrollViewDelegate {
    ...
    func configure(with sliderMedia: [SliderMedia], width: CGFloat, height: CGFloat, currentPage: Int) {
        ...
        for (index, media) in sliderMedia.enumerated() {
            ...
            if media.type == .image && media.image == nil {
                let photoButtonOverlay = UIImageView(frame: CGRect(x: width * CGFloat(index),
                                                          y: 0,
                                                          width: width,
                                                          height: height))
                ...
                scrollView.addSubview(photoButtonOverlay)
            }
            
            if media.type == .video {
                let playButtonOverlay = UIImageView(frame: CGRect(x: width * CGFloat(index),
                                                          y: 0,
                                                          width: width,
                                                          height: height))

                let targetWidth = width * 0.20
        ...
        var contentSizeWidth = width * CGFloat(sliderMedia.count)
        if sliderMedia.count == 1 {
            contentSizeWidth -= 1 // disable extra scrolling for single image
        }
        scrollView.contentSize = CGSize(width: contentSizeWidth, height: height)
