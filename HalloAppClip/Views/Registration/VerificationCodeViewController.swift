...
class VerificationCodeViewController: UIViewController, UITextFieldDelegate {
    ...
    private func getAppUpdateAlertController() -> UIAlertController {
        let alert = UIAlertController(
            ...
            preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: Localizations.buttonUpdateRegistration, style: .default, handler: { action in
            DDLogInfo("VerificationCodeViewController/updateNotice/update clicked")
            guard let customAppURL = AppContextCommon.appStoreURL,
                  UIApplication.shared.canOpenURL(customAppURL) else
            {
                DDLogError("VerificationCodeViewController/updateNotice/error unable to open [\(AppContextCommon.appStoreURL?.absoluteString ?? "nil")]")
                return
            }
