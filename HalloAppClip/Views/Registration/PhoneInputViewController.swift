...
class PhoneInputViewController: UIViewController, UITextFieldDelegate {
    ...
    let phoneNumberKit: PhoneNumberKit = AppContextCommon.shared.phoneNumberFormatter
    let textFieldPhoneNumber = PhoneNumberTextField(withPhoneNumberKit: AppContextCommon.shared.phoneNumberFormatter)
    ...
    override func viewDidLoad() {
        ...
        cancellableSet.insert(
            AppContextCommon.shared.didGetGroupInviteToken.sink { [weak self] in
        ...
        buttonSignIn.setTitle(Localizations.buttonNextRegistration, for: .normal)
        ...
        if UserDefaults.standard.bool(forKey: Self.deletedAccountKey) == true {
            ...
            alert.addAction(UIAlertAction(title: Localizations.buttonOKRegistration, style: .default, handler: { _ in
                exit(0)
            }))
    ...
    private func getGroupNameIfNeeded() {
        guard let groupInviteToken = AppContextCommon.shared.userData.groupInviteToken else {
            return
        }
        delegate?.getGroupName(groupInviteToken: groupInviteToken) { [weak self] result in
            ...
            case .success(let groupName):
                if let groupName = groupName {
                    ...
                    AppContextCommon.shared.userData.groupName = groupName
